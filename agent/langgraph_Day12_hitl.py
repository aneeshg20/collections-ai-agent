...
def pre_classify(state: InvoiceState):
    overdue_flag = bool(state['days_since_invoice'] > state['payment_term_days'])
...
def send_communication(state: InvoiceState):
    """The actual send action - only runs AFTER human approval."""
    print(f"\n✅ COMMUNICATION SENT for {state['vendor']}")
    print(f"   Action: {state['recommended_action']}")
    print(f"   (In production: email dispatched via Gmail/SMTP)")
    return {}
...
def check_ageing(state: InvoiceState):
    ...
    if vendor_data.empty:
        return{"aging_risk_flag": False, "aging_summary": "No aging data found"}
    ...
    aging_risk_flag = bool(bucket_61_90 > 0 or bucket_90_plus > 0)
...
def check_disputes(state: InvoiceState):
    ...
    dispute_flag = bool(count_open_disputes > 0)
    return {
                "dispute_flag": dispute_flag,
                "dispute_count": int(count_open_disputes)
    }
...
def check_ptp(state: InvoiceState):
    ...
    ptp_broken_flag = bool(count_ptp_broken >= 2)
    return{
            "ptp_broken_count": int(count_ptp_broken),
...
def check_credit(state: InvoiceState):
    ...
    credit_risk_flag = bool(utilization_pct > 80)
    return{
            "credit_utilization": float(utilization_pct),
...
def check_vendor_master(state: InvoiceState):
    ...
    vendor_risk_flag = bool(vendor_tier == "At-Risk" or payment_score < 50)
    return{
            "vendor_tier": str(vendor_tier),
...
# START goes to conditional dispatch (the orchestrator)
...
# Aggregator continues to pre_classify
...
graph.add_node("send_communication", send_communication)
graph.add_conditional_edges(
...
graph.add_edge("log_telemetry", "send_communication")
graph.add_edge("send_communication", END)

# Set up SQLite checkpointer for state persistence
conn_checkpoint = sqlite3.connect("checkpoints.db", check_same_thread=False)
checkpointer = SqliteSaver(conn_checkpoint)

# Compile with interrupt BEFORE send_communication
app = graph.compile(
    checkpointer=checkpointer,
    interrupt_before=["send_communication"]
)
...
# Process just ONE invoice to understand HITL clearly
data = pd.read_csv("agent/invoices.csv")
row = data.iloc[1]  # Honeywell (the HIGH risk one)
...
# Unique thread ID for this invoice run
config = {"configurable": {"thread_id": "honeywell_inv_001"}}

print("=" * 60)
print("PHASE 1: Running agent until human approval gate...")
print("=" * 60)

# First invoke - runs until interrupt_before send_communication
result = app.invoke(invoice, config)

# At this point the graph has PAUSED before send_communication
print(f"\n📋 DRAFTED COMMUNICATION (awaiting approval):")
print(f"Vendor: {result['vendor']}")
print(f"Risk: {result['risk_rating']}")
print(f"Action: {result['recommended_action']}")
print(f"\n{result['drafted_communication'][:500]}")

print("\n" + "=" * 60)
print("AGENT PAUSED. Simulating human review...")
print("=" * 60)

# Simulate human decision
human_decision = input("\nApprove and send? (yes/no): ").strip().lower()

if human_decision == "yes":
    print("\nPHASE 2: Human approved. Resuming agent...")
    # Resume by invoking with None - checkpointer continues from pause
    final_result = app.invoke(None, config)
    print("\n✅ Workflow complete - communication sent.")
else:
    print("\n❌ Human rejected. Communication NOT sent.")
    print("   (In production: route to re-draft or manual handling)")                       

# After run - show full telemetry
conn = sqlite3.connect("collections.db")
summary = pd.read_sql("""
