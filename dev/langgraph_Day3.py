...
def assess_risk(state: InvoiceState):
    ...
    max_tokens=1024,
    system = """You are an agent in a world class collections team for a managed services account in a top tier consultancy firm.
...
graph.add_edge("assess_risk","log_telemetry")
...
telemetry_result = pd.read_sql("""
            SELECT * FROM agent_runs ORDER BY run_id LIMIT 1""",conn)
