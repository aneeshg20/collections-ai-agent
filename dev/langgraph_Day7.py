...
def collection_strategy(state: InvoiceState):
    ...
    Promise to Pay Flag: {state['ptp_risk_flag']}
    Promis to Pay broken count: {state['ptp_broken_count']}
    Credit limit nearing flag: {state['credit_risk_flag']}
    Credit utilization value: {state['credit_utilization']}
...
graph.add_edge("check_credit","pre_classify")
